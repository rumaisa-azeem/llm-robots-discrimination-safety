...
class CCA(_CCABase):
    ...
    def __init__(self, reg=0., numCC=10, kernelcca=True, ktype=None,
                 verbose=True, cutoff=1e-15, lengthscale=1.0):
        super(CCA, self).__init__(reg=reg, numCC=numCC, kernelcca=kernelcca,
                                  ktype=ktype, verbose=verbose, cutoff=cutoff, gausigma=lengthscale)
...
def kcca(data, reg=0., numCC=None, kernelcca=True, ktype='linear',
    ...
    LH = GraphLaplacian(LH) + 1e-5 * np.eye(LH.shape[0])
    RH = GraphLaplacian(RH) + 1e-5 * np.eye(RH.shape[0])
